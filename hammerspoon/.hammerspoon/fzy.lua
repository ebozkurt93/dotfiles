-- Slightly modified version of https://github.com/swarn/fzy-lua/blob/main/src/fzy_lua.lua
...
-- Function to filter and sort a table of tables based on a key
-- Args:
--   needle (string)
--   haystack (table): A table of tables, where each sub-table has the key to be searched
--   search_key (string): The key in the sub-tables to search against
--   case_sensitive (bool, optional): defaults to false
--
-- Returns:
--   table: Sorted table of matching items, each item is {item = originalItem, score = matchingScore}
function fzy.filter_table(needle, haystack, search_key, case_sensitive)
  local matches = {}

  for _, item in ipairs(haystack) do
    local value = item[search_key]
    if type(value) == "string" and fzy.has_match(needle, value, case_sensitive) then
      local _, score = fzy.positions(needle, value, case_sensitive)
