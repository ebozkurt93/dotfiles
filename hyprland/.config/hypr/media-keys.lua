-- Hardware media keys (volume/brightness/playback). macOS handles these
-- natively (nothing to port out of Hammerspoon -- it never rebinds them), so
-- these are new binds, not a port. Hyprland itself is just a compositor and
-- has no built-in handling for them, unlike a full DE (GNOME/KDE), so
-- without these binds the keys do nothing.
-- "-l 1.0" caps wpctl volume at 100% so repeated presses can't boost past
-- unity gain.
