...
Item {
    ...
    // Quickshell's native UPowerDevice binding doesn't expose charge-cycle
    // count (not all backends report it, and it's not in the Q_PROPERTY
    // list), so this one field is worth a real shell-out to `upower -i`
    // rather than skipping it -- same field Omarchy's own power panel
    // surfaces via its own battery-status script.
