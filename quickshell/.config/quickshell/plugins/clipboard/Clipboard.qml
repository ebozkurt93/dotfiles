...
// Clipboard history panel backed by cliphist, with a two-pane list+preview UI (not the Launcher's flat list).
PanelWindow {
    id: root
    property var shell
    ...
    // Piped through clipboard-capture (which drops oversized pastes) rather than straight into `cliphist store`.
    // Restarted if it ever dies, since a dead watcher takes clipboard history with it silently.
    Process {
        id: watchProcess
